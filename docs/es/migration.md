...
Hay tres formas de migrar datos desde una instalación existente de RisuAI (RisuAI Web, RisuAI Local) a RisuBard. Elija según su entorno de origen y el tamaño de los datos.
...
Exporte un archivo de copia de seguridad `.bin` desde RisuAI existente y luego impórtelo en RisuBard. Funciona independientemente del entorno de origen (web / Tauri / Capacitor / local).

1. **En RisuAI existente**: Configuración > Copia de seguridad > "Guardar copia local" para exportar un archivo `.bin`.
2. **En RisuBard**: Configuración > Migración de datos > "Importar copia local de Risu original" para importar el archivo `.bin`.
...
2. En RisuBard, vaya a Configuración > Migración de datos > acordeón "Importar carpeta save de NodeOnly Risu".
...
1. Detenga el servidor RisuBard.
2. Sobrescriba la carpeta `save` de RisuBard con la carpeta `save` del RisuAI existente.
3. Reinicie el servidor RisuBard — la migración automática comienza.
