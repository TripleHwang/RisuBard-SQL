...
1. Open [GitHub Releases](https://github.com/TripleHwang/RisuVault/releases).
2. Download and extract the package for your platform.
3. Start RisuBard and open `http://localhost:6001`.

| Platform | Package | Start |
| --- | --- | --- |
| Windows x64 | `RisuVault-vX.Y.Z-win-x64.zip` | Double-click `RisuVault.exe` |
| Linux x64 | `RisuVault-vX.Y.Z-linux-x64.tar.gz` | Run `./start.sh` |
| Linux ARM64 | `RisuVault-vX.Y.Z-linux-arm64.tar.gz` | Run `./start.sh` |
| macOS Apple Silicon | `RisuVault-vX.Y.Z-macos-arm64.tar.gz` | Open `RisuVault.app` |
...
RisuVault is under active development. Keep a current backup before migrating important data, and check the [release notes](https://github.com/TripleHwang/RisuVault/releases) for changes that affect storage or compatibility.
