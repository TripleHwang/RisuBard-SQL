...
const INJECTION_KINDS = new Set([
    'systemPrompt', 'jailbreak', 'globalNote', 'authorNote', 'character',
    'persona', 'lorebook', 'wiki', 'memory', 'exampleDialogue',
    'chatHistory', 'instruction', 'tool', 'other',
]);
...
function normalizeInjectionManifest(value) {
    ...
    const items = raw.items.slice(0, 256).flatMap((item) => {
        ...
        const kind = INJECTION_KINDS.has(item.kind) ? item.kind : 'other';
        const tokens = toNonNegInt(item.tokens);
        return [{
            kind,
            ...(item.name != null ? { name: str(item.name, 256) } : {}),
            tokens,
        }];
    });
    return {
        totalTokens: toNonNegInt(raw.totalTokens),
        items,
...
function normalizeEntry(entry) {
    ...
    const injectionManifestText = injectionManifest ? JSON.stringify(injectionManifest) : null;

    const sizeBytes = Buffer.byteLength(headers.text ?? '', 'utf8')
        + Buffer.byteLength(body.text ?? '', 'utf8')
        + Buffer.byteLength(response.text ?? '', 'utf8')
        + Buffer.byteLength(injectionManifestText ?? '', 'utf8');
    ...
    return {
        ...
        sessionChatId: str(entry.sessionChatId, 128),
        ...
        injectionManifest: injectionManifestText,
...
function rowToEntry(r) {
    return {
        ...
        injectionManifest: normalizeInjectionManifest(r.injection_manifest),
...
const LIST_COLUMNS = `
    id, timestamp, category, source, chat_id, session_chat_id, generation_id, model, provider, url, method,
    status, success, aborted, route, streaming, duration_ms, first_token_ms,
    input_tokens, output_tokens, cached_tokens, reasoning_tokens, injection_manifest,
...
function createRequestLogs(opts = {}) {
    ...
    // Existing installs predate per-chat evidence metadata. SQLite's
    ...
    if (!requestColumns.has('session_chat_id')) {
        db.exec(`ALTER TABLE requests ADD COLUMN session_chat_id TEXT`);
    }
    if (!requestColumns.has('injection_manifest')) {
        db.exec(`ALTER TABLE requests ADD COLUMN injection_manifest TEXT`);
    }
    db.exec(`CREATE INDEX IF NOT EXISTS idx_requests_session_chat ON requests(session_chat_id)`);

    const stmtInsertRequest = db.prepare(`
        INSERT INTO requests (
            timestamp, category, source, chat_id, session_chat_id, generation_id, model, provider,
            ...
            @timestamp, @category, @source, @chatId, @sessionChatId, @generationId, @model, @provider,
            @url, @method, @status, @success, @aborted, @route, @streaming,
            @durationMs, @firstTokenMs,
            @inputTokens, @outputTokens, @cachedTokens, @reasoningTokens, @injectionManifest,
    ...
    // ─── Query: request log ──────────────────────────────────────────────────
    function buildRequestWhere({ categories, sources, chatId, sessionChatId, successOnly, failedOnly, since, until } = {}) {
        const conditions = [];
        const params = [];
        ...
        if (sessionChatId) {
            conditions.push(`session_chat_id = ?`);
            params.push(String(sessionChatId));
        }
        ...
        app.get('/api/request-logs', async (req, res, next) => {
            ...
            try {
                const filterArgs = {
                    ...
                    sessionChatId: typeof req.query.session_chat_id === 'string'
                        ? req.query.session_chat_id
                        : undefined,
