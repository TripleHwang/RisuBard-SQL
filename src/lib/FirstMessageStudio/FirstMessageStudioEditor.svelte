<script lang="ts">
    ...
    let ImageCropEditor = $state<undefined | typeof import('./FirstMessageStudioImageCropEditor.svelte').default>(undefined)
    ...
    function loadImageCropEditor() {
        if (ImageCropEditor) return
        void import('./FirstMessageStudioImageCropEditor.svelte').then(({ default: component }) => ImageCropEditor = component)
    }

    $effect(() => {
        if (presentationOption?.presentation?.imageAssetName) loadImageCropEditor()
    })
    ...
    async function selectStudioFiles(allowedExtensions: string[]) {
        const { selectFileByDom } = await import('src/ts/util')
        return selectFileByDom(allowedExtensions)
    }
    ...
    async function uploadPresentationImage(option: FirstMessageStudioOption) {
        const files = await selectStudioFiles(['png', 'webp', 'jpeg', 'jpg', 'gif', 'avif'])
        const file = files?.[0]
        if (!file) return
        const extension = file.name.split('.').pop()?.toLowerCase() || 'png'
        const assetName = uniquePresentationAssetName(option, file.name)
        const { saveAsset } = await import('src/ts/globalApi.svelte')
    ...
    async function exportProject() {
        ...
        const { downloadFile } = await import('src/ts/globalApi.svelte')
    ...
    async function importProject() {
        shareMessage = ''
        try {
            const files = await selectStudioFiles(['json'])
            if (!files?.[0]) return
        ...
        <main class="workspace">
            ...
            <section class="editor">
                <nav class="mode-tabs" data-studio-primary-toolbar aria-label="스튜디오 편집 영역">
                    <button type="button" data-studio-content-tab class:active={editorMode === 'content'} onclick={() => editorMode = 'content'}>화면과 선택지</button>
                ...
                <section class="project-settings-toolbar" aria-label="프로젝트 설정 도구">
                    <button
                        type="button"
                        data-studio-project-settings
                        aria-expanded={showProjectSettings}
                        aria-controls="studio-project-settings-panel"
                        onclick={toggleProjectSettings}
                    >프로젝트 설정</button>
                </section>
                {#if showProjectSettings}
                    <section id="studio-project-settings-panel" class="form-box three-columns" data-studio-project-settings-panel aria-label="프로젝트 설정">
                        <label>창 제목<input value={localized(draft.title)} oninput={(event) => draft.title = setLocalized(draft.title, event.currentTarget.value)}/></label>
                        ...
                        {:else}
                            <div class="variable-list">
                                ...
                                {#each draft.variables as variable, variableIndex}
                                    <section class="variable-entry" data-studio-variable={variable.name}>
                                    ...
                                    <div class="variable-choice-heading">
                                        <strong>선택 가능한 값</strong>
                                        <button type="button" data-studio-add-variable-choice onclick={() => addVariableChoice(variable)}>＋ 값 추가</button>
                                    </div>
                                    {#if variable.choices.length > 0}
                                        <div class="variable-choice-list">
                                            {#each variable.choices as choice, choiceIndex}
                                                <div class="variable-choice-row" data-studio-variable-choice>
                                    ...
                                    {/if}
                                    <button class="use-variable-choices" type="button" disabled={variable.choices.length === 0} onclick={() => useVariableChoices(variable)}>현재 화면에 이 값들을 선택지로 추가</button>
                                    </section>
                                {/each}
                            </div>
                        {/if}
                                        ...
                                        <div class="presentation-fields" role="tabpanel">
                                            <div class="presentation-image-controls">
                                                ...
                                                {#if presentation.imageEnabled}
                                                    ...
                                                    {#if presentation.imageAssetName && ImageCropEditor}
                                                        <ImageCropEditor
...
<style>
    ...
    .project-settings-toolbar{display:flex;justify-content:flex-end;margin-bottom:.55rem}.project-settings-toolbar button{border:1px solid var(--risu-theme-darkborderc);font-size:.7rem;font-weight:800}.project-settings-toolbar button[aria-expanded='true']{border-color:var(--risu-theme-primary);color:var(--risu-theme-primary)}
    ...
    .variable-list{display:grid;gap:.38rem;overflow-x:auto;padding:.15rem}.variable-list-header,.variable-row{display:grid;grid-template-columns:minmax(9rem,1fr) minmax(9rem,1fr) minmax(7rem,1fr) 2.4rem 2.4rem 3.6rem;gap:.42rem;min-width:38rem}.variable-list-header{padding:0 .05rem;color:var(--risu-theme-textcolor2);font-size:.63rem;font-weight:800}.variable-list-header span[title]{cursor:help}.variable-entry{display:grid;gap:.45rem;min-width:38rem;padding:.5rem;border:1px solid var(--risu-theme-darkborderc);border-radius:.5rem;background:var(--risu-theme-darkbg)}.variable-row{align-items:center}.variable-row input{min-width:0}.variable-row button{height:100%;min-height:2.15rem;border:1px solid var(--risu-theme-darkborderc);border-radius:.38rem;font-weight:800}.variable-row button:not(:disabled):hover{border-color:var(--risu-theme-primary);color:var(--risu-theme-primary)}.variable-row .delete{color:var(--color-danger)}.variable-choice-heading{display:flex;align-items:center;justify-content:space-between;gap:.5rem;padding-top:.25rem;border-top:1px solid var(--risu-theme-darkborderc);font-size:.68rem}.variable-choice-heading button,.use-variable-choices{justify-self:start;border:1px solid var(--risu-theme-primary);border-radius:.38rem;color:var(--risu-theme-primary);font-size:.66rem;font-weight:800}.variable-choice-list{display:grid;gap:.35rem}.variable-choice-row{display:grid;grid-template-columns:minmax(9rem,1fr) auto minmax(9rem,1fr) auto;align-items:center;gap:.35rem}.variable-choice-row button{color:var(--color-danger)}.use-variable-choices:disabled{opacity:.5}.empty-state{padding:1rem;border:1px dashed var(--risu-theme-darkborderc);border-radius:.6rem;color:var(--risu-theme-textcolor2);font-size:.72rem;text-align:center}
