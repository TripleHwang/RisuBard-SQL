...
describe('atomic character selection contract', () => {
    test('hydrates the character and bounded active chat before committing the selected ID', () => {
        ...
        expect(deferredSelection).toContain('onFailure: (error) =>')
        expect(deferredSelection).toContain("alertError(error ?? new Error('Unable to load the selected character. Please try again.'))")
        const failureHandler = deferredSelection.slice(deferredSelection.indexOf('onFailure: (error) =>'), deferredSelection.indexOf('},\n    })'))
