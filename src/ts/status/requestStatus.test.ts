...
describe('publish API', () => {
    it('keeps the input injection manifest on the request entry', () => {
        const manifest = {
            totalTokens: 42,
            items: [{ kind: 'systemPrompt', name: 'Main', tokens: 42 }],
        } satisfies RequestInjectionManifest
        startStatus('g1', {
            kind: 'main',
            label: 'gpt',
            now: 100,
        ...
        expect(get(requestStatuses).get('g1')?.injectionManifest).toEqual(manifest)
    })
...
describe('input injection manifest', () => {
    it('merges provenance when request normalization combines messages', () => {
        expect(mergeRequestStatusSources(
            [{ kind: 'systemPrompt', role: 'system', content: 'system' }],
            [{ kind: 'lorebook', name: 'Moon archive', role: 'system', content: 'lore' }],
        ).map((source: { kind: string }) => source.kind)).toEqual([
            'systemPrompt',
            'lorebook',
        ])
    })

    it('allocates exact message tokens across named sources and aggregates matching items', async () => {
            ...
            [
                {
                    ...
                    requestStatusSources: [
                        { kind: 'systemPrompt', name: 'Main', role: 'system', content: 'aaaa' },
                        { kind: 'lorebook', name: 'Moon archive', role: 'system', content: 'bbbbbb' },
                    ],
                },
                {
                    role: 'system',
                    content: 'more lore',
                    requestStatusSources: [
                        { kind: 'lorebook', name: 'Moon archive', role: 'system', content: 'cc' },
                    ],
                },
        ...
        expect(manifest.totalTokens).toBe(23)
        expect(manifest.estimated).toBe(true)
        expect(manifest.items).toEqual([
            { kind: 'systemPrompt', name: 'Main', tokens: 5 },
            { kind: 'lorebook', name: 'Moon archive', tokens: 10 },
            { kind: 'chatHistory', tokens: 8 },
        ])
    ...
    it('reconciles the input manifest to authoritative provider prompt usage', () => {
        startStatus('g1', {
            ...
            injectionManifest: {
                ...
                estimated: true,
                items: [
                    { kind: 'systemPrompt', tokens: 5 },
                    { kind: 'chatHistory', tokens: 15 },
                ],
            },
        })
        endStatus('g1', 'done', {
        ...
        const manifest = get(requestStatuses).get('g1')?.injectionManifest
        expect(manifest).toEqual({
            totalTokens: 40,
            estimated: false,
            items: [
                { kind: 'systemPrompt', tokens: 10 },
                { kind: 'chatHistory', tokens: 30 },
            ],
        })
    })
})
