...
function compilePresentations(project: FirstMessageStudioProject, stage: FirstMessageStudioProject['stages'][number], stageIndex: number): string {
    ...
    const fallbackOption = stage.options.find((option) => hasFirstMessageStudioPresentation(option.presentation)) ?? stage.options[0]
    const presentationOptions = stage.options.map((option) => hasFirstMessageStudioPresentation(option.presentation) ? option : fallbackOption)
    const presentations = presentationOptions.map((option, optionIndex) => {
    ...
    const positionCss = presentationOptions.map((option, optionIndex) => {
        const presentation = option.presentation
