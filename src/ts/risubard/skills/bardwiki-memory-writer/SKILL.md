...
- 정체성, 능력, 관계, 목표, 제약, 지속 상태처럼 이후 행동이나 해석을 바꿀 정보는 높은 가치를 둔다. 일회성 장식, 중복 정보, 근거 없는 추론은 제외한다.
- 후보 수를 임의로 줄이는 것을 품질로 간주하지 마라. 후보 예산 안에서는 중요한 정보의 누락 비용과 과잉 기록 비용을 비교해 적극적으로 보존하라.
- 응답 전에 확정 본문의 지속 정보가 기존 정본 또는 후보 어디에도 대표되지 않은 채 빠졌는지 점검하라. 특정 항목 유형을 기계적으로 전부 기록하지 말고 서사적 효용을 기준으로 판단하라.
