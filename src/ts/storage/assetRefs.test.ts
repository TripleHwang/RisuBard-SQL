...
import {
    ...
    characterAssetReferencesComplete,
...
describe('character asset reference completeness', () => {
    test('treats a metadata-first summary character as an incomplete reference set', () => {
        expect(characterAssetReferencesComplete([{ detailsLoaded: true }, { detailsLoaded: false }])).toBe(false)
    })

    test('accepts characters that are fully loaded or carry no hydration marker', () => {
        expect(characterAssetReferencesComplete([{ detailsLoaded: true }, {}])).toBe(true)
        expect(characterAssetReferencesComplete([])).toBe(true)
    })

    test('refuses deletion when the character list itself is unavailable', () => {
        expect(characterAssetReferencesComplete(undefined)).toBe(false)
    })
})
