...
import type {
  ...
  BotPresetSummary,
  StoredBotPreset,
  SqlRevision as NodePostgresRevision,
  SqlRevisionDetails as NodePostgresRevisionDetails,
  SqlRevisionDiff as NodePostgresRevisionDiff,
  SqlRestorePreview as NodePostgresRestorePreview,
  SqlMessageSearchResult as NodePostgresMessageSearchResult,
  SqlTokenUsage as NodePostgresTokenUsage,
  SqlCharacterSearchResult as NodePostgresCharacterSearchResult,
...
const DB_FILE = "/risu-standalone.sqlite3";
...
export class WebSqliteStorage implements ISqlStorage {
  ...
  private loadMessageValue(chatId: string, messageId: string): Message {
    const message = (this.loadNodeValue(
  ...
  async loadDatabase(
    ...
    // Compatibility is the release gate. The first standalone backend returns
    // the complete legacy object graph; shallow loading is enabled only after
    // the same import/export suite passes against metadata proxies.
    void options;
    const shallow = false;
    ...
    for (const row of charRows) {
      if (shallow) {
        ...
        for (const cr of chatRows) {
          ...
          cd.message = await this.loadChatMessages(cr.id as string);
    ...
    const activePresetId = (db as any).activeBotPresetId as string | undefined;
    const presetSummaries = await this.listBotPresets();
    db.botPresets = (
      await Promise.all(
        presetSummaries.map((summary) => this.loadBotPreset(summary.id)),
      )
    ).filter((preset): preset is StoredBotPreset => preset !== null);
    db.botPresetsId = Math.max(
      0,
      db.botPresets.findIndex((preset) => preset.id === activePresetId),
    );
    delete (db as any).activeBotPresetId;
  ...
  async loadChat(
    ...
    cd.message = mr.map((r) =>
      this.loadMessageValue(chatId, r.id as string),
    );
  ...
  async loadChatMessages(chatId: string): Promise<Message[]> {
    return this.selectRows(
      "SELECT id FROM messages WHERE chat_id = ? ORDER BY position",
      [chatId],
    ).map((r) => this.loadMessageValue(chatId, r.id as string));
  }
  ...
  async loadChatMessagePage(
    ...
    return {
      messages: rows.map((row) =>
        this.loadMessageValue(chatId, row.id as string),
      ),
      offset,
      total,
      hasMore: offset > 0,
