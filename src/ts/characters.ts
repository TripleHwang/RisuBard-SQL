...
export async function exportChat(page:number){
    try {
        ...
        if(hydratedChat?._placeholder || hydratedChat?.messagesFullyLoaded === false || getSqlWindow(hydratedChat)?.hasOlder){
...
export async function exportAllChats() {
    try {
        ...
        for (let i = 0; i < char.chats.length; i++) {
            ...
            if (chat?._placeholder || chat?.messagesFullyLoaded === false || getSqlWindow(chat)?.hasOlder) {
